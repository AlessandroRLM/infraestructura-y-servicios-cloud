...
- **Administración de cuentas y roles (`IamService`):** un administrador (`users.manage`) lista y busca usuarios (`ListUsers`, paginación keyset + búsqueda por email/nombre), consulta el detalle de uno (`GetUser`) y asigna o revoca roles (`AssignRole`/`RevokeRole`). Toda mutación de rol queda registrada en `audit_logs` (`role.assign` / `role.revoke`). Dos resguardos protegen contra escalada de privilegios sobre el rol admin: un administrador no puede quitarse a sí mismo el rol admin (auto-degradación), y no se puede revocar el último administrador del sistema —este verificado de forma atómica con bloqueo de fila (`SELECT … FOR UPDATE`) dentro de la transacción de revocación—. El ciclo de vida de cuenta (alta/invitación, deshabilitación vía `users.disabled_at`, reset de contraseña) queda para una fase posterior.
