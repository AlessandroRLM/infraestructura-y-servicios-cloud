/**
 * StudentRecordReportView component tests — RF-9.3, AC-3.b, AC-3.e, AC-4.a..4.e, AC-5.a, AC-6.a
 *
 * renderPdf seam is mocked so no real WASM runs in happy-dom.
 * Transport is stubbed via makeStubTransport (covers ReportsService + IamService for StudentPicker).
 */
...
import { IamService, ListUsersResponseSchema } from "@/gen/iam/v1/iam_pb";
import {
...
function renderStudentRecord(
  ...
  route = "/admin/reports?tab=student-record&studentId=student-uuid-1",
  iamImpl: IamImpl = defaultIamImpl,
) {
...
describe("StudentRecordReportView — states", () => {
  it("AC-4.a: no studentId in URL → no-filter prompt visible", async () => {
    renderWithProviders({
      route: "/admin/reports?tab=student-record",
      transport: makeStubTransport(
        [ReportsService, {}],
        [IamService, defaultIamImpl],
      ),
      session: adminSession,
      sessionSource: adminSessionSource,
    });
  ...
  it("ready state: renderReportPdf is called with a non-null ReportPdfModel (AC-3.e)", async () => {
    ...
    renderStudentRecord({
      getStudentRecordReport: async () => makeSuccessResponse(),
    });
    ...
    expect(calledWith?.title).toBe("Expediente de Alumno");
  ...
  it("AC-4.c: empty response → empty message, no iframe, no download link", async () => {
    renderStudentRecord({
      getStudentRecordReport: async () =>
        create(GetStudentRecordReportResponseSchema, {
          studentId: "student-uuid-1",
          studentName: "Ana García",
          rows: [],
          generatedAt: "2026-06-18T10:00:00Z",
