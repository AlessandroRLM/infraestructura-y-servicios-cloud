...
func TestLoad(t *testing.T) {
	...
	tests = append(tests, struct {
		...
		name: "missing_metrics_auth_token_is_error",
		env: map[string]string{
			"DATABASE_URL":      testDBURL,
			"REDIS_URL":         testRedisURL,
			"SESSION_TTL":       "3600s",
			...
			"RESET_TOKEN_TTL":   "900s",
			"REPORTS_CACHE_TTL": "5m",
			// METRICS_AUTH_TOKEN intentionally absent
		},
		wantErr: true,
	}, struct {
		name    string
		...
		name: "metrics_auth_token_present",
		env: map[string]string{
			"DATABASE_URL":       testDBURL,
			"REDIS_URL":          testRedisURL,
			"SESSION_TTL":        "3600s",
	...
	allKeys := []string{
		...
		"REPORTS_CACHE_TTL", "METRICS_AUTH_TOKEN",
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if cfg.MetricsAuthToken != tt.wantCfg.MetricsAuthToken {
				t.Errorf("MetricsAuthToken = %q, want %q", cfg.MetricsAuthToken, tt.wantCfg.MetricsAuthToken)
			}
		})
	}
}
